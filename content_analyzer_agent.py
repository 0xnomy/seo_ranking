...
class ContentAnalyzerAgent:
    ...
    def analyze_content(self, seo_data: dict) -> dict:
        ...
        try:
            ...
            prompt = f"""
            Analyze the following JSON for content structure and readability. Output ONLY:
            - A markdown table with heading tags and their counts (reference JSON keys).
            - The total number of paragraphs and their average length.
            - A list of missing or weak headings (if any).
            ...
            while retry_count < Config.MAX_RETRIES:
                try:
                    response = self.groq_client.chat.completions.create(
                        ...
                        messages=[
                            {"role": "system", "content": "You are an expert content structure analyst."},
                            {"role": "user", "content": prompt}
                        ],
                        max_tokens=500
                    )
                    analysis = response.choices[0].message.content.strip()
                    # Save output in the same JSON file
                    seo_data['content_analysis'] = analysis
                    ...
                    return {"content_analysis_complete": True, "file": filepath}
            ...
            log_error(f"analyze_content error: {str(e)}", agent="ContentAnalyzerAgent")
            return {"error": str(e)} 